...
export default function Home() {
  ...
  const highRisk = weather.humidity > 70 && weather.rain;
      ...
      { image: base64 },
      {
        onSuccess: (data) => {
          sessionStorage.setItem(
            "lastAnalysis",
            JSON.stringify({ result: data, image: base64 })
          );
          setLocation("/analysis/current");
        },
      }
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f5] px-4 pb-28">
      <div className="max-w-3xl mx-auto space-y-6 pt-6">

        {/* WEATHER CARD */}
        <div className="relative rounded-3xl bg-gradient-to-r from-emerald-50 to-lime-50 border border-emerald-200 overflow-hidden">
          <div className="p-6">
            ...
            {highRisk && (
              <div className="mt-4 flex items-center gap-2 bg-orange-50 border border-orange-200 text-orange-700 px-4 py-2 rounded-xl text-sm font-semibold">
                <AlertTriangle className="w-4 h-4" />
                HIGH RISK: Fungal disease possible due to rain
              </div>
            )}

            <div className="mt-3 bg-emerald-100 text-emerald-800 px-4 py-2 rounded-xl text-sm font-semibold">
              👨‍🌾 Best action today: Spray neem oil before evening
            </div>
          </div>
        </div>
        ...
        <motion.button
          ...
          className="w-full bg-gradient-to-r from-emerald-700 to-green-600 text-white py-5 rounded-full text-lg font-bold flex items-center justify-center gap-3 shadow-lg"
        >
        ...
        <div className="grid grid-cols-3 gap-3">
          <Feature icon={<ShieldCheck />} label="Prevention" />
          <Feature icon={<CloudRain />} label="Weather Alerts" />
          <Feature icon={<Headphones />} label="Expert Help" />
        </div>
        ...
        <div className="bg-white rounded-3xl p-5 shadow-sm border">
          ...
          <ul className="space-y-2 text-sm text-slate-700">
            <li>📅 Yesterday · High humidity warning</li>
            <li>✅ Aug 10 · Spray reminder completed</li>
            <li>🌱 Aug 8 · Disease check – Healthy</li>
          </ul>

          <button
            onClick={() => setLocation("/helpline")}
            className="mt-4 w-full bg-emerald-700 text-white py-2 rounded-xl font-semibold"
          >
            Expert Help
          </button>
        </div>
        ...
        {history && history.length > 0 && (
          <div
            onClick={() => setLocation("/analysis/view")}
            className="cursor-pointer bg-white border rounded-2xl p-5"
          >
            ...
            <p className="text-sm text-slate-500">
              {formatDistanceToNow(new Date(history[0].timestamp), {
                addSuffix: true,
              })}
            </p>
          </div>
...
function Feature({
  ...
  return (
    <div className="bg-white rounded-2xl p-4 flex flex-col items-center gap-2 shadow-sm border">
